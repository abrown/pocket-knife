...
class Service{
    ...
    */
    public function execute(){
    	...
    	try{
    		// apply acl
    		if( !$this->allowed('class', $class) ) throw new ExceptionAccess('Class is not allowed', 403);
    		if( !$this->allowed('method', $method) ) throw new ExceptionAccess('Method is not allowed', 403);
    		//if( !$this->allowed('id', $id) ) throw new ExceptionAccess('ID is not allowed', 403);
    		// get instance
    		if( !$this->object ){
    			if( !class_exists($this->{'class'}) ) throw new ExceptionFile('Could not find class: '.$this->{'class'}, 404);
    			if( !in_array('ServiceObject', class_implements($this->{'class'})) ) throw new ExceptionConfiguration('Class must implement ServiceObject.', 500);
    		...
    		// do method
    		if( !method_exists($this->object, $this->method) ) throw new ExceptionConfiguration('Method does not exist', 404);
    ...
    public function getRouting(){
		...
		if( !$method ){
			...
			// case: object/numeric_id
				...
				if( $http_method ) $$method = $http_method;
				else $method = 'read';
			}
			// case: entities/
			else $method = 'read';
    ...
    **/
    public function allowed($type, $value){
		// allow/deny settings
		if( is_bool($this->acl) ) return $this->acl;
		// get list
		static $list = null;
		if( is_null($list) ){
			$list = (array) $this->acl;
    ...
    **/
    protected function getStorage(){
		...
		if( !$object ){
			...
			if( !array_key_exists('type', $configuration) ) throw new ExceptionConfiguration('Storage type is not defined', 500);
    ...
    **/
    ...
    protected function getContentClass($content_type){
    	...
    	// return
    	if( array_key_exists($content_type, $map) ) return $map[$content_type];
    	else throw new ExceptionConfiguration('Attempting to use unknown content type: '.$content_type, 500);
    }
}