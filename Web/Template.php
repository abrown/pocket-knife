...
class WebTemplate{
	...
	public function __construct($input, $type = self::FILE){
		if( !is_string($input) ) throw new ExceptionSettings('Template input must be a string', 500);
		if( !is_integer($type) ) throw new ExceptionSettings('Template input type must be an integer (see class constants)', 500);
		$this->input = $input;
		$this->type = $type;
	}
