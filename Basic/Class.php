...
class BasicClass {
    ...
    public static function autoloadAll($class) {
        ...
        foreach ($interfaces as $_class) {
            if (!interface_exists($_class))
                autoload($_class);
        }
        // then rest of classes
        foreach ($dependencies as $_class) {
            if (!class_exists($_class) && !interface_exists($_class))
                autoload($_class);
        }
        // return
        return true;
    ...
    public static function findDependencies($class, $dependencies = array()) {
        ...
        if (preg_match('/@uses (.*)/', $content, $matches)) {
            ...
            foreach ($classes as $_class) {
                ...
                if (in_array($_class, $dependencies)) {
                    // remove from current location: needed sooner
                    $dependencies = array_diff($dependencies, array($_class));
                    // prepend to front of array
                    array_unshift($dependencies, $_class);
                    continue;
                } else {
                    // prepend to front of array
                    array_unshift($dependencies, $_class);
